# Chiron Sung HK: Source Files

Source files of Chiron Sung HK.
...
```bash
docker build -t chiron-sung-builder:latest .
```
...
```bash 
docker run --rm -it -v $(pwd):/source -v /path/to/build/output:/build chiron-sung-builder
```
...
```bash 
docker run --rm -it -v $(pwd):/source --entrypoint /source/bin/build_var_gf.sh chiron-sung-builder
```
